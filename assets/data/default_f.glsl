...
void main() {
    ...
    for (int j = 0; j < iterations; ++j)
	    ...
	    vec2 coord1 = reflect(MyArray[j],rand)*rad;
	...
	vec3 len2 = vec3(1.0 - len);
	//finalColor.rgb = diffuse.rgb + vec3(1.0 - len) - ao;
	//finalColor.rgb = finalColor.rgb;
	finalColor.rgb += (diffuse.rgb / 1.0) + len2 - (ao * 1.0) + 0.1;
	//finalColor.rgb += (diffuse.rgb / 2.0) + len2 - (ao * 2.0) + 0.3;
	//finalColor.rgb += diffuse.rgb - (ao / 1.0) + len2;
	//finalColor.rgb += diffuse.rgb + len2 - ao;
	vec2 coord1 = reflect(MyArray[0],rand)*rad;
	vec2 coord2 = vec2(coord1.x*0.707 - coord1.y*0.707,coord1.x*0.707 + coord1.y*0.707);
	

	vec3 diff = getPosition(v_texCoords0 + coord1*0.25).xyz - p;
	vec3 v = normalize(diff);
	float d = length(diff)* 0.1;//g_scale;
	float max = max(0.0,dot(n,v) + 0.0) * (1.0 / (1.0 + d));
	float ao2;
	ao2 += max;
	//ao += doAmbientOcclusion(v_texCoords0,coord1*0.25, p, n);
	float z;
 	z = texture2D(u_depthMap, v_texCoords0).a; //v_texCoords0
	//finalColor.rgb += z;
